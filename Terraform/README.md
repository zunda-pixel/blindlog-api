...
terraform apply \
  ...
  -target=google_secret_manager_secret.otel_collector_config \
  -target=google_secret_manager_secret_version.otel_collector_config
```

初回 bootstrap では、Secret Manager に `latest` version が存在しない状態で Cloud Run を作ると失敗する。ここでは API 有効化、アプリ用 secret の入れ物作成、OTel Collector 設定 version の作成を先に済ませる。
OTel Collector 設定は非機密のため、`Deploy/cloud-run/collector-config.yaml` から Terraform が Secret Manager version を作る。
...
Cloud Run の env / scaling / resources / OTel Collector 設定などは `terraform.tfvars`、`*.tf`、`Deploy/cloud-run/collector-config.yaml` を編集して `terraform apply`。GHA の image 更新とは独立している (app image は `ignore_changes`)。
...
アプリ用 Secret は `version = "latest"` 参照なので、次回リビジョン作成時に新しい値が読まれる。すぐ反映させたい場合は GHA deploy を回すか、`gcloud run services update blindlog-api --region=$REGION --update-env-vars=_DUMMY=$(date +%s)` のような空更新で新リビジョンを作る。
