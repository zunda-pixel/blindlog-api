...
struct RouterTests {
  ...
  @Test
  func updateEventClearsLifecycleDatesWhenOmittedOrNull() async throws {
    let arguments = TestArguments()
    let app = try await buildApplication(arguments)
    let ipAddress = UUID().uuidString
    let initialDate = 1_000_000.0
    let updatedDate = 2_000_000.0
    ...
    try await app.test(.router) { client in
      ...
      func eventRequest(
        title: String,
        startsAt: Double,
        answersPublishedAt: Double? = nil,
        capacity: Int32? = nil,
        entryFee: Components.Schemas.Money? = nil,
        visibility: Components.Schemas.EventVisibility,
        ...
        body: ByteBuffer(
          data: JSONEncoder().encode(
            eventRequest(
              ...
              answersPublishedAt: initialDate,
              capacity: 12,
              entryFee: .init(minorAmount: 1_500, currencyCode: "JPY"),
              visibility: ._public,
              publishedAt: initialDate,
              canceledAt: initialDate
            )
          )
        )
      ) { response in
      ...
      #expect(created.answersPublishedAt == initialDate)
      #expect(created.capacity == 12)
      #expect(created.entryFee?.minorAmount == 1_500)
      #expect(created.publishedAt == initialDate)
      #expect(created.canceledAt == initialDate)

      let omitted = try await updateEvent(
        eventID: created.id,
        body: eventRequest(
          title: "Omitted lifecycle event",
          startsAt: initialDate + 10_000,
          visibility: .unlisted
        )
      )
      #expect(omitted.title == "Omitted lifecycle event")
      #expect(omitted.answersPublishedAt == nil)
      #expect(omitted.capacity == nil)
      #expect(omitted.entryFee == nil)
      #expect(omitted.publishedAt == nil)
      #expect(omitted.canceledAt == nil)

      let updated = try await updateEvent(
        eventID: created.id,
        body: eventRequest(
          title: "Updated lifecycle event",
          startsAt: updatedDate,
          answersPublishedAt: updatedDate,
          capacity: 8,
          entryFee: .init(minorAmount: 2_000, currencyCode: "JPY"),
          visibility: ._public,
          publishedAt: updatedDate,
          canceledAt: updatedDate
        )
      )
      #expect(updated.answersPublishedAt == updatedDate)
      #expect(updated.capacity == 8)
      #expect(updated.entryFee?.minorAmount == 2_000)
      #expect(updated.publishedAt == updatedDate)
      #expect(updated.canceledAt == updatedDate)

      let nullBody = try JSONSerialization.data(withJSONObject: [
        "title": "Null lifecycle event",
        "body": "Null lifecycle event body",
        "venueName": "Blind Tasting Room",
        "venueAddress": [
          "addressLine1": "1 Wine Street",
          "countryCode": "JP",
        ],
        "eventPeriod": [
          "startsAt": updatedDate + 10_000,
          "endsAt": updatedDate + 13_600,
        ],
        "answersPublishedAt": NSNull(),
        "visibility": "public",
        "publishedAt": NSNull(),
        "canceledAt": NSNull(),
      ])
      let nullCleared = try await updateEvent(eventID: created.id, bodyData: nullBody)
      #expect(nullCleared.title == "Null lifecycle event")
      #expect(nullCleared.answersPublishedAt == nil)
      #expect(nullCleared.capacity == nil)
      #expect(nullCleared.entryFee == nil)
      #expect(nullCleared.publishedAt == nil)
      #expect(nullCleared.canceledAt == nil)
    }
  }
